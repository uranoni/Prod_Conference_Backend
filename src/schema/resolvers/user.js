...
export default {
    ...
    Mutation: {
        updateUser: async (root, args, ctx) => {
            if (!ctx.user) return new AuthenticationError('You must be logged in to edit the user!')
            console.log(ctx.user)
            if (ctx.user.access.role !== 'admin' || ctx.user.access.group !== 'center') return new ForbiddenError("Permission Denied")
            try {
